...
bool User::loadPersonalConfigFromFile() {
    ...
    if (plik.is_open()) {
        ...
        for (int i = 0; i < vec.size(); i += 5) {
            transactions_.push_back(std::make_shared<Transaction>(std::stoi(vec[i]), vec[i + 1], std::stod(vec[i + 2]), static_cast<ExpenseCategory>(std::stoi(vec[i + 3])), Date::DateFromString(vec[i + 4])));
        }
    ...
    if (plik2.is_open()) {
        ...
        for (int i = 0; i < vec2.size();) {
            if (vec2[i] == "0" || vec2[i] == "1" || vec2[i] == "2" || vec2[i] == "3") {
                obligations_.push_back(std::make_shared<Bills>(static_cast<BillType>(std::stoi(vec2[i])), std::stod(vec2[i + 1]), Date::DateFromString(vec2[i + 2])));
                i += 3;
            } else {
                obligations_.push_back(std::make_shared<Loan>(vec2[i], std::stod(vec2[i + 1]), Date::DateFromString(vec2[i + 2]), std::stoi(vec2[i + 3])));
                i += 4;
            }
        }
        return true;
