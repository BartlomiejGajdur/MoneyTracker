...
void Menu_Main::ObligationsMenuRun(User& User) {
    ...
    case 3:
        ...
        User.payBills();
        temp > User.getCurrentMoney() ? std::cout << MenuFunctions::SetTextColor(Color::Green, "All obligations have been paid!\n") : std::cout << MenuFunctions::SetTextColor(Color::Red, "No obligations to paid found!\n");
        MenuFunctions::WaitForAction();
        break;
...
void Menu_Main::greetUser(const User& User) {
    ...
    User.getCurrentMoney() >= 0 ? std::cout << MenuFunctions::SetTextColor(Color::Green, "+" + moneyToDisplay) : std::cout << MenuFunctions::SetTextColor(Color::Red, moneyToDisplay);
    std::cout << std::endl;
...
void Menu_Main::removeTransactionByID_Menu(User& user) {
    ...
    user.removeTransactionById(choice) == UserErrorCode::Ok ? std::cout << MenuFunctions::SetTextColor(Color::Green, "Transaction with given ID removed correctly :)\n") : std::cout << MenuFunctions::SetTextColor(Color::Red, "Transaction with given ID have not been found ;/\n");
    MenuFunctions::WaitForAction();
}
